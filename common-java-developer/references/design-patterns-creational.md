...
**Why:** Without `volatile`, JIT instruction reordering can publish a partially-constructed instance to another thread. Use enum or static holder instead. See [pitfalls-antipatterns.md](pitfalls-antipatterns.md#-double-checked-locking-without-volatile) for the full anti-pattern.
